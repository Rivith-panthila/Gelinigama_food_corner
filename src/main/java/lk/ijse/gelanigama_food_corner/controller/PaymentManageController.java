...
//import lk.ijse.gelanigama_food_corner.controller.models.PaymentModel;
...
public class PaymentManageController implements Initializable {
    ...
    //SupplierBO supplierBO=(SupplierBO) BoFactory.getInstance().getBO(BoFactory.BOType.SUPPLIER);
        PaymentBO paymentBO=(PaymentBO) BoFactory.getInstance().getBO(BoFactory.BOType.PAYMENTS);
    ...
    private void loadNextPaymentId() {
        try {
            ...
            String formattedId = "PAY" + String.format("%04d", nextId);
            paymentIdTxt.setText(formattedId);
            System.out.println("Payment ID set to: " + formattedId);  // ← Debug
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void processPayment() {
        if (paymentIdTxt.getText() == null || paymentIdTxt.getText().isEmpty()) {
            loadNextPaymentId();
        }
...
//        pay paymentModel = new PaymentModel();
        paymentBO.handlePrintBill(orderId);
    } catch (Exception e) {
        e.printStackTrace();
